...
def walk_list(request):
    ...
    walks_list = postwalk.objects.filter(authorised=True).order_by(
        *order_by_fields
    )
    ...
    return render(
        request, "walks/walk_list.html", {"walks": walks, "sort": sort}
    )
...
def walk_detail(request, slug):
    ...
    return render(
        request,
        "walks/walk_detail.html",
        {"walk": walk, "comments": comments, "comment_form": comment_form},
    )


# Home page
def index(request):
    featured_walks = postwalk.objects.filter(
        featured=True, authorised=True
    ).order_by("-date_added")[:3]
    walk_count = postwalk.objects.filter(
        authorised=True
    ).count()  # Counts only approved walks
    total_distance = (
        postwalk.objects.filter(authorised=True).aggregate(Sum("distance"))[
            "distance__sum"
        ]
        or 0
    )  # Sums only approved walks
...
def walk_create(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.success(
                request,
                "Your walk has been submitted and is awaiting approval!",
            )
...
def comment_edit(request, comment_id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.success(
                request,
                "Comment updated! It will be visible once approved by an admin.",
            )
            return redirect("walk_detail", slug=comment.walk.slug)
...
def favourite_walks(request):
    """View to display user's favourite walks"""
    walks = request.user.favourite_walks.filter(authorised=True).order_by(
        "-date_added"
    )
